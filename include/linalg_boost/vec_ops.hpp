...
namespace wheel::linalg_boost::detail {
     ...
     * @note Returns 0 when either vector is a zero vector
     */
    inline void batch_cosine_similarity_scalar(const float **a, const float *b, size_t n, size_t batch_size, float *results) {
        ...
        if (bb == 0.0f) {
            // If b is a zero vector, all similarities are 0
     ...
     * @note Returns 0 when either vector is a zero vector
     */
    inline void batch_cosine_similarity_neon(const float **a, const float *b, size_t n, size_t batch_size, float *results) {
...
#ifdef LINALG_USE_ASM
    /**
     * @brief Optimized Assembly implementation of batch cosine similarity for AArch64
     *
     * @param a Array of vector pointers
     * @param b Pointer to the reference vector
     * @param n Size of each vector
     ...
     * @note Returns 0 when either vector is a zero vector
     */
    inline void batch_cosine_similarity_asm_aarch64(const float **a, const float *b, size_t n, size_t batch_size, float *results) {
        ...
        for (size_t k = 0; k < aligned_batch_size; k += batch_step) {
            ...
            if (blocks) {
                ...
                asm volatile(
                    ...
                    : [dot0] "=&r"(dot0), [dot1] "=&r"(dot1), 
                      [aa0] "=&r"(aa0), [aa1] "=&r"(aa1),
                      [pa0] "+r"(pa0), [pa1] "+r"(pa1), [pb] "+r"(pb), 
